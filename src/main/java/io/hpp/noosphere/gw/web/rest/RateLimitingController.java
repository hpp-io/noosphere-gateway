...
public class RateLimitingController {
  ...
  @GetMapping("/configs")
  public Flux<RateLimitConfig> getConfigs(Pageable pageable) {
    return rateLimitingService.getConfigCache(pageable);
  }

  @PostMapping("/configs/{key}")
  public Mono<ResponseEntity<Void>> updateConfig(@PathVariable String key, @RequestBody RateLimitConfig config) {
  ...
  public Mono<ResponseEntity<Void>> cacheAllRateLimitConfigs() {
    return rateLimitingService.cacheAllRateLimitConfigs().then(Mono.just(ResponseEntity.ok().build()));
  }
}
