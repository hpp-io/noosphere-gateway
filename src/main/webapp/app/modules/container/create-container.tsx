...
import { createContainer } from './container.reducer';
...
export const CreateContainer = () => {
  ...
  const validateForm = () => {
    ...
    if (!containerData.imageName.trim()) {
      errors.imageName = 'Container image name is required';
    }

    if (!containerData.walletAddress.trim()) {
      errors.walletAddress = 'Wallet address is required';
    } else if (!/^0x[a-fA-F0-9]{40}$/.test(containerData.walletAddress)) {
      errors.walletAddress = 'Invalid wallet address format';
    }

    if (!containerData.port.trim()) {
      errors.port = 'Port is required';
    } else if (isNaN(Number(containerData.port))) {
      errors.port = 'Port must be a valid number';
    } else {
      const portNum = Number(containerData.port);
    ...
    const invalidVolumes = nonEmptyVolumes.filter(volume => {
      // Basic validation for Docker volume format (host_path:container_path or just container_path)
      const volumePattern = /^([a-zA-Z0-9._/-]+:)?[a-zA-Z0-9._/-]+$/;
      return !volumePattern.test(volume.trim());
    });

    if (invalidVolumes.length > 0) {
      errors.volumes = 'Volume entries must follow the format "host_path:container_path" or just "container_path"';
    }
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    const submitData = {
      ...
      name: containerData.name || null,
      walletAddress: containerData.walletAddress,
      imageName: containerData.imageName,
      command: containerData.command || undefined,
  ...
  return (
    <div className="create-container container-form-section">
      <h2 className="text-center my-4">Create New Container</h2>
          ...
          <Col md={12}>
            <FormGroup>
              ...
              <Input
                ...
                name="walletAddress"
                placeholder="0x..."
                value={containerData.walletAddress}
                onChange={handleInputChange}
                ...
                <CardBody>
                    ...
                    <Row key={index} className="mb-2 align-items-end">
                      <Col md={10}>
                        ...
                        <Input
                          type="text"
                          id={`volume-${index}`}
                          placeholder="e.g., /host/path:/container/path or /container/path"
                          value={volume}
                  ...
                  <div className="text-muted small mt-2">
                    ...
                    <ul className="mb-0 mt-1">
                      <li><code>/host/data:/app/data</code> - Bind mount host directory to container</li>
